...
class Futurable<T> {
  ...
  then = <R1 = T, R2 = never>(
    onFulfilled: (value: R1 | T) => void | R1 | Futurable<R1> = (x: R1 | T) =>
      x as R1,
    onRejected: (reason?: any) => void | R2 | Futurable<R2> = x => {
      throw x
    }
  ) => {
    return new Futurable<T>((resolve, reject) => {
      ...
      this.resolvers.push({
        ...
        handleCatch: value => rejectOnError(() => onRejected(value)),
      })
  ...
  private resolve = (value: void | T | Futurable<T>) => {
   ...
   */
  private setResult = runAsyncFn((value: any, state: FuturableState) => {
